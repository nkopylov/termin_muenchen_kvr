...
async def check_and_notify(application: Application) -> None:
    ...
    while True:
        try:
            ...
            # Check for appointments for all unique date ranges
            date_ranges = {}
            ...
            # Check each unique date range
            for date_key, user_ids in date_ranges.items():
                start_date, end_date = date_key.split('_')

                logger.info(f"Checking for appointments {start_date} to {end_date} (users: {len(user_ids)})")
                ...
                if appointments_found:
                    logger.info(f"✅ Appointments found! Notifying {len(user_ids)} users")
                    ...
                    # Format the message
                    message = (
                        "🎉 <b>APPOINTMENTS AVAILABLE!</b> 🎉\n\n"
                        "Appointments are now available for:\n"
                        "<b>Notfalltermin Ausländerbehörde München</b>\n\n"
                    )

                    if appointments_detail:
                        message += f"Available slots:\n{appointments_detail}\n"
                    else:
                    ...
                    logger.info(f"No appointments available for {start_date} to {end_date}")
                    stats['successful_checks'] += 1
                    stats['last_success_time'] = datetime.now()
                    consecutive_failures = 0
...
def main() -> None:
    ...
    logger.info("Bot started! Waiting for users to subscribe...")

    # Load existing subscribers
    subscribers = load_subscribers()
    logger.info(f"Loaded {len(subscribers)} existing subscribers from database")
